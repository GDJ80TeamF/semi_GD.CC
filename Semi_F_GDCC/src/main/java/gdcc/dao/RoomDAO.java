...
public class RoomDAO {
	...
	public static int updateRoomState(int roomNo, String roomState) throws Exception{
		...
		String sql = "UPDATE rooms_hotel SET room_state =? WHERE room_no= ?";
		PreparedStatement stmt = null;
		stmt = conn.prepareStatement(sql);
	...
	public static HashMap<String, Object> selectRoom(int roomNo) throws Exception {
		...
		if(rs.next()) {
			...
			resultMaps.put("roomGrade", rs.getInt("roomGrade"));
	...
	// 룸 수정
	public static int updateRoom(int roomNo, int roomGrade, int roomPrice, 
			int roomMax, String roomState, String roomImg) throws Exception {
		int row =0;
		
		// db 연결
		Connection conn = DBHelper.getConnection();
		
		String sql = "UPDATE rooms_hotel SET room_grade =?, "
